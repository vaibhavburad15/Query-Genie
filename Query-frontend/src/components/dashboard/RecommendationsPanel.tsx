...
const RecommendationsPanel: React.FC<RecommendationsPanelProps> = ({
  ...
  const loadRecommendations = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`http://https://query-genie-h0cy.onrender.com/api/recommendations/${userId}`);
  ...
  const handleClick = async (rec: Recommendation) => {
    ...
    if (rec.type === 'template' && rec.id) {
      try {
        await fetch(`http://https://query-genie-h0cy.onrender.com/api/recommendations/${rec.id}/use`, {
          method: 'POST',
        });
      } catch (error) {
