...
📌 METADATA QUERIES (CRITICAL):
   When user asks about database structure, use INFORMATION_SCHEMA:
...
- "show all tables" / "list all tables" / "what tables" / "show me all table" → 
  SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE()

- "show all data from X" / "all rows in X" / "everything in X table" → 
  SELECT * FROM X

- "describe X table" / "columns in X" / "structure of X" →
...
❌ NEVER do these:
1. Don't use SHOW TABLES (use INFORMATION_SCHEMA.TABLES instead)
2. Don't use DESCRIBE table (use INFORMATION_SCHEMA.COLUMNS instead)
3. Don't forget GROUP BY with aggregates
4. Don't use = NULL (use IS NULL)
5. Don't compare datetime without DATE() function
6. Don't create joins without ON condition
7. Don't use ambiguous column names in multi-table queries
...
Q: "List all tables in the database"
A: SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE()

Q: "What columns are in the products table?"
A: SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'products'

Q: "Show me all data from products table"
...
CRITICAL: Always use INFORMATION_SCHEMA for metadata queries, never SHOW commands.
"""